...
def _import_images(
    ...
    with pycolmap.ostream():
        image_list_str = [str(img) for img in image_list] if image_list else []
...
def _get_image_ids(
    database_path: Path, input_folder: Path
    ...
    for name, image_id in db.execute("SELECT name, image_id FROM images;"):
        ...
        feature_image_ids[image_path.as_posix()] = image_id
        image_ids[name] = image_id
    db.close()
    return image_ids, feature_image_ids
...
def reconstruct(
    ...
    image_ids, feature_image_ids = _get_image_ids(database, image_dir)
    import_features(feature_image_ids, database, features)
    import_matches(
        feature_image_ids,
        database,
        pairs,
    ...
    reconstruction = _run_reconstruction(sfm_dir, database, image_dir, verbose, mapper_options)
